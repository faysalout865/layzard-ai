...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "Lyzard AI" }: AuthComponentProps) => {
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
        ...
        try {
            ...
            setModalErrorMessage(err.message || "Failed to sign up. Please try again.");
            setModalStatus('error');
        }
    }
  };
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      setModalErrorMessage(err.message || 'Google sign in failed. Please try again.');
      setModalStatus('error');
    }
  };
                ...
                <AnimatePresence mode="wait">
                    {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                        ...
                        <BlurFade delay={0.25 * 3}><div className="flex items-center justify-center gap-4 w-full">
                            <GlassButton onClick={handleGoogleSignIn} type="button" contentClassName="flex items-center justify-center gap-2" size="sm"><GoogleIcon /><span className="font-semibold text-white">Google</span></GlassButton>
                        </div></BlurFade>
                    </motion.div>}
